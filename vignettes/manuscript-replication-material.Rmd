...
I think the estimation part correct.
...
a= predict(cox.fit,newdata=d_test[1],times=tmp,cause=1,product.limit = FALSE)
b= predict(cox.fit,newdata=d_test[1],times=tmp,cause=1,product.limit = TRUE)

x= predict(cox.fit,newdata=d_test[1],times=tmp,cause=1,type="survival",product.limit = FALSE)
y= predict(cox.fit,newdata=d_test[1],times=tmp,cause=1,type="survival",product.limit = TRUE)

z = predict(out,newdata=d_test[1],times=tmp,cause=1,absolute_risk_integration_method = "approx")

w = predict(out,newdata=d_test[1],times=tmp,cause=1,absolute_risk_integration_method = "exact")
...
max(x$survival-z$survival_function)

# much different than product.limit=TRUE
max(x$survival-y$survival)
...
b_score_1 = Score(comparables, #
    ...
    data=d_test,times=tmp[10:12], cause=1)
```
